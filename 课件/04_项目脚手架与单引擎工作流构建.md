...
| 2 | 增加数据库和 LLM 配置 | 在 `Settings` 中补充本步骤需要的 DB、Insight LLM、输出目录配置 |
| 3 | 实现最小本地搜索工具 | 先只实现 `search_topic_globally`，用于跑通本地数据库查询到 `DBResponse` 的最小闭环 |
| 4 | 构建LangGraph工作流 | 定义LangGraph的state类，实现生成结构、初始搜索、初始总结、反思搜索、反思总结、格式化报告、保存报告等节点代码 |
...
```text
InsightEngine 节点 -> InsightContext.execute_search() -> search_topic_globally -> DBResponse -> summary 节点
```
...
```mermaid
flowchart LR
    A[InsightEngine 节点] --> B[InsightContext.execute_search]
...
图中描述的是本地搜索工具的调用边界：节点只调用 `InsightContext.execute_search()`，由 `InsightContext` 根据工具名分发到 `MediaCrawlerDB` 的具体方法。`MediaCrawlerDB` 负责查询本地数据库，并把不同平台、不同表结构的数据统一转换为 `DBResponse`。节点后续只处理统一结果，不关心底层 SQL 和平台表差异。
...
flowchart TD
    ...
    D --> F
    F --> G[调用 InsightContext.execute_search]
...
| 节点不直接写 SQL | 节点只调用 `InsightContext.execute_search()`，不直接接触数据库 |
...
| 序号 | 任务 | 说明 |
| --- | --- | --- |
| 1 | 增加情感分析配置 | 在 `Settings` 中加入情感分析开关和模型配置 |
| 2 | 增加聚类配置 | 在 `Settings` 中加入聚类开关、模型名和采样上限 |
| 3 | 实现情感分析工具 | 创建 `sentiment_analyzer.py`，支持懒加载和失败降级 |
| 4 | 实现聚类工具 | 创建 `clustering.py`，对大量搜索结果聚类采样 |
| 5 | 改造 InsightContext | 在 `execute_search()` 中串联去重、聚类、情感分析 |
| 6 | 添加降级测试 | 验证模型不可用时主研究流程仍然可继续 |

### 6.3 本步骤新增配置

| 配置项 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `SENTIMENT_ANALYSIS_ENABLED` | bool | `true` | 是否启用情感分析 |
| `ENABLE_SENTIMENT_PER_SEARCH` | bool | `true` | 每次搜索是否默认做情感分析 |
| `SENTIMENT_MODEL_NAME` | string | 模型名 | 情感分析模型 |
| `ENABLE_CLUSTERING` | bool | `true` | 是否启用聚类 |
| `CLUSTERING_MODEL_NAME` | string | 模型名 | 聚类嵌入模型 |
| `MAX_CLUSTERED_RESULTS` | int | `50` | 聚类后最大结果数 |
| `RESULTS_PER_CLUSTER` | int | `5` | 每个簇采样数量 |

### 6.4 本步骤新增/改造文件

```text
engines/InsightEngine/
├── tools/
...
flowchart TD
    A[本地搜索工具返回结果] --> B[结果去重]
    B --> C{ENABLE_CLUSTERING?}
    C -- 是 --> D[聚类与采样]
    C -- 否 --> E[保留去重结果]
    D --> F{SENTIMENT_ANALYSIS_ENABLED?}
    E --> F
    F -- 是 --> G[情感分析]
    F -- 否 --> H[返回 DBResponse]
    G --> H
...
图中描述的是第 06 步对第 05 步搜索结果处理流程的增强。搜索工具先返回原始结果，系统先做去重，避免相同 URL 或相同内容重复进入后续分析。之后根据 `ENABLE_CLUSTERING` 判断是否执行聚类采样；聚类完成或跳过聚类后，再根据 `SENTIMENT_ANALYSIS_ENABLED` 判断是否执行情感分析。无论增强能力是否开启，最终都必须返回统一的 `DBResponse`，保证第 04 步已经完成的主研究链路不被破坏。

### 6.6 情感分析输出

情感分析不是单独生成一个文件，它的产物会挂载到本次搜索工具的 `DBResponse.parameters["sentiment_analysis"]` 中，供后续段落总结节点读取。

情感分析产物包含两层：

- 单条文本的情感判断：每条搜索结果对应一个情感标签和置信度。
- 整体情感摘要：对本次搜索结果的正面、负面、中性分布做统计。

单条情感结果结构：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `text` | string | 被分析的原始文本 |
| `sentiment_label` | string | 情感标签 |
| `confidence` | float | 置信度 |
| `probability_distribution` | object | 各标签概率 |
| `success` | bool | 单条是否成功 |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `total_analyzed` | int | 实际参与情感分析的文本数量 |
| `label_counts` | object | 各情感标签数量 |
| `label_ratios` | object | 各情感标签占比 |
| `average_confidence` | float | 平均置信度 |
| `dominant_sentiment` | string | 占比最高的情感标签 |
| `results` | list | 单条情感结果列表 |

情感分析写入 `DBResponse` 后的示例：
...
| 使用位置 | 使用方式 |
| --- | --- |
| `initial_summary` | 在总结搜索结果时引用 `dominant_sentiment`、`label_ratios`，说明整体情绪倾向 |
| `reflection_summary` | 在补充材料后重新判断情绪是否发生变化 |
| `format_report` | 可在最终报告中输出“情绪概览”小节 |

### 6.7 聚类输出

聚类采样的目标不是生成额外报告，而是把大量重复或相似的搜索结果压缩成一批更适合 LLM 阅读的代表性材料。

聚类的最终产物有两个：

- `DBResponse.results`：被替换为去重、聚类、采样后的代表性结果列表。
- `DBResponse.parameters["clustering"]`：记录聚类过程的摘要，便于调试和报告追踪。

聚类输入输出：

| 项目 | 说明 |
| --- | --- |
| 输入 | 本地搜索工具返回的 `list[QueryResult]` |
| 输出 | 采样后的 `list[QueryResult]`，仍然写回 `DBResponse.results` |
| 去重依据 | URL 优先；无 URL 时使用内容前 100 字符 |
| 采样目标 | 保留不同观点/不同主题下的代表性内容，减少重复文本 |
| 降级 | 聚类模型不可用时跳过聚类，返回去重结果 |

聚类摘要结构：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `enabled` | bool | 是否执行了聚类 |
| `original_count` | int | 原始结果数量 |
| `deduplicated_count` | int | 去重后数量 |
| `cluster_count` | int | 聚类数量 |
| `sampled_count` | int | 最终采样数量 |
| `clusters` | list | 每个簇的摘要信息 |
...
| `cluster_id` | int | 聚类编号 |
| `size` | int | 该簇原始结果数量 |
| `sampled_count` | int | 该簇被采样的结果数量 |
| `keywords` | list[string] | 该簇代表关键词，可选 |
| `representative_text` | string | 该簇代表文本 |

聚类写入 `DBResponse` 后的示例：
      ...
      "clusters": [
        {
          ...
          "sampled_count": 5,
          "keywords": ["售后", "客服", "退款"],
          "representative_text": "售后客服一直没有处理退款问题"
        },
        {
          "cluster_id": 1,
          "size": 24,
          "sampled_count": 5,
          "keywords": ["质量", "故障", "维修"],
          "representative_text": "产品用了两天就出现故障，维修周期太长"
        }
      ]
    }
  },
  ...
  "results": [
    {
      "platform": "weibo",
      "content_type": "comment",
      "title_or_content": "售后客服一直没有处理退款问题",
      "author_nickname": "用户A",
      "url": null,
      "publish_time": "2026-05-19T10:00:00",
      "engagement": {
...
| 阶段 | 数量 | 说明 |
| --- | --- | --- |
| 原始搜索结果 | 120 | 多平台搜索结果，可能包含大量重复转发、相似评论 |
| 去重后结果 | 86 | 去掉相同 URL 或高度重复文本 |
| 聚类后主题簇 | 4 | 例如“售后响应慢”“产品质量问题”“官方回应不足”“用户观望” |
| 最终采样结果 | 20 | 每个主题簇保留若干代表性文本，供 LLM 总结 |

段落总结节点使用方式：

| 使用位置 | 使用方式 |
| --- | --- |
| `initial_summary` | 使用采样后的 `results`，避免把大量重复内容塞给 LLM |
| `reflection_summary` | 根据聚类摘要判断是否存在新的观点簇 |
| `format_report` | 可在报告中说明主要讨论主题来自哪些聚类 |

### 6.8 与 InsightContext 的集成

| 步骤 | 行为 |
| --- | --- |
| 1 | `execute_search()` 调用本地搜索工具 |
| 2 | 对结果去重 |
| 3 | 如果开启聚类，执行聚类采样 |
| 4 | 如果开启情感分析，执行情感分析 |
| 5 | 将聚类摘要写入 `DBResponse.parameters["clustering"]`，将情感分析摘要写入 `DBResponse.parameters["sentiment_analysis"]` |
...
| --- | --- |
| 模型加载 | 情感分析和聚类模型必须懒加载 |
| 缺少依赖 | 缺少模型依赖时降级，不影响 04 和 05 已完成的主流程 |
| 测试边界 | 单元测试使用 FakeSentimentAnalyzer 和 FakeClusteringService |
| 性能边界 | 大量结果必须先采样再送入 LLM |

### 6.10 边界情况

| 场景 | 处理方式 |
| --- | --- |
| 情感模型下载失败 | 禁用情感分析，主流程继续 |
| 聚类模型不可用 | 跳过聚类，主流程继续 |
| 文本为空 | 跳过或标记为分析失败 |
| 单条分析失败 | 记录失败项，批量继续 |
| 搜索结果过多 | 聚类采样或按配置截断 |
...
| 测试文件 | 测试目标 | Mock/前置条件 | 操作 | 预期结果 |
| --- | --- | --- | --- | --- |
| `tests/test_insight_engine_e2e.py` | 验证情感分析产物写入 | 使用 FakeSentimentAnalyzer | 调用 `InsightContext.execute_search()` | `DBResponse.parameters["sentiment_analysis"]` 存在，包含 `dominant_sentiment`、`label_counts` |
| `tests/test_insight_engine_e2e.py` | 验证单条情感结果结构 | FakeSentimentAnalyzer 返回固定标签 | 执行情感分析 | 每条结果包含 `text`、`sentiment_label`、`confidence`、`success` |
| `tests/test_insight_engine_e2e.py` | 验证情感模型不可用降级 | FakeSentimentAnalyzer 标记不可用 | 执行研究链路 | 主流程继续，不因情感分析失败中断 |
| `tests/test_insight_engine_e2e.py` | 验证聚类产物写入 | 使用 FakeClusteringService | 调用 `InsightContext.execute_search()` | `DBResponse.parameters["clustering"]` 存在，包含 `original_count`、`sampled_count` |
| `tests/test_insight_engine_e2e.py` | 验证聚类采样结果数量 | 准备超过 `MAX_CLUSTERED_RESULTS` 的 fake 搜索结果 | 执行聚类采样 | `DBResponse.results` 数量不超过配置上限 |
| `tests/test_insight_engine_e2e.py` | 验证聚类模型不可用降级 | FakeClusteringService 抛异常或返回不可用 | 执行研究链路 | 返回去重结果，主流程继续 |
| `tests/test_insight_engine_e2e.py` | 验证去重规则 | 准备重复 URL 和重复内容数据 | 执行 `execute_search()` | 重复项被去除，保留代表性结果 |
| `tests/test_insight_engine_e2e.py` | 验证增强能力关闭 | 设置 `SENTIMENT_ANALYSIS_ENABLED=false`、`ENABLE_CLUSTERING=false` | 执行研究链路 | 不写入对应增强产物，主流程仍完成 |
...
| 验收项 | 验收方式 |
| --- | --- |
| 01 骨架可用 | `python main.py` 能启动，`GET /api/system/status` 成功 |
| 02 API 脚手架可用 | `/api/config`、`/api/search`、`/api/search/latest` 返回符合设计 |
| 03 SSE 可用 | `/api/events/stream` 能连接并收到 connected/keepalive |
| 04 InsightEngine 主流程可用 | `POST /api/search` 能启动 InsightEngine，并通过 SSE 输出进度 |
| 05 本地搜索工具扩展可用 | 扩展搜索工具均返回统一 `DBResponse`，异常入参能返回明确错误 |
| 06 情感/聚类增强可用 | 开启或关闭增强能力时，InsightEngine 主流程都能完成或明确报错 |

---

## 08. 本阶段必须通过的测试命令

本阶段测试命令用于验证 01-06 的后端基础能力和 InsightEngine 单引擎工作流。AI Coding 完成任一子模块后，应至少运行对应单项测试；完成本阶段全部内容后，应运行组合回归命令。

| 验证范围 | 命令 | 必须验证的内容 |
| --- | --- | --- |
| 后端工具函数 | `pytest tests/test_app_utils.py -q` | 配置读取、路径处理、基础工具函数行为稳定 |
| 后端服务层 | `pytest tests/test_app_services.py -q` | ConfigService、SearchService、ResultService 等服务层逻辑可用 |
| InsightEngine 单引擎链路 | `pytest tests/test_insight_engine_e2e.py -q` | 本地搜索工具、`DBResponse`、研究链路、情感分析、聚类采样 |
| 本阶段组合回归 | `pytest tests/test_app_utils.py tests/test_app_services.py tests/test_insight_engine_e2e.py -q` | 验证 01-06 完整阶段没有相互破坏 |
...
| 回归优先级 | 修改服务层或 Engine 时，必须运行本阶段组合回归命令 |
